    ...
    <nav class="navbar navbar-expand-md navbar-light bg-light sticky-top">
        ...
        <ul style="width: 100%; text-align:center; background-color: #3366FF;  ">
            ...
            <div class="dropdown">
                ...
                <div class="dropdown-content">
                    <a href="view-teachingPlan.php">Kế hoạch giảng dạy dự kiến</a>
                    <a href="view-teachingNow.php">Lịch trình giảng dạy thực tế</a>
                </div>
            </div>
        </ul>
    </nav>
    <div style="margin-bottom: 40px;" class="text-center main-content">
        <h2 style="margin-bottom:30px;">Teaching Plan</h2>
        <?php
            require('connectdb.php');
            //B2:
            $sql = 'SELECT teachingPlanId, teacherLastName, teacherFirstName, subjectName, className, Lesson, Place, Date FROM teachingplan, teacher, subject, teachassigned, class WHERE (teacher.teacherid = teachassigned.teacherid) AND (subject.subjectid = teachassigned.subjectid) AND (class.classid = teachassigned.classid) AND (teachingplan.teachAssignedId = teachassigned.teachAssignedId)';
            mysqli_set_charset($dbcon, 'utf8');
            $result = mysqli_query($dbcon,$sql);
            //B3:
            echo '<font size="5">';
            echo '<table style="border: 2px solid"; align="center";>';
            echo '<tr style="border: 2px solid;"><th style="border: 2px solid">ID</th><th style="border: 2px solid">Teacher Name</th><th style="border: 2px solid">Class Name</th><th style="border: 2px solid">Subject Name</th><th style="border: 2px solid">Lesson</th><th style="border: 2px solid">Place</th><th style="border: 2px solid">Date</th>';
            if(mysqli_num_rows($result)>0){
            while($row = mysqli_fetch_assoc($result)){
                echo '<tr style="border: 2px solid">';
                echo '<td style="border: 2px solid">'.$row['teachingPlanId'].'</td>';
