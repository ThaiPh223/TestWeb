...
<body>
    ...
    <div class="text-center main-content">
        <h2 style="margin-bottom:30px;">List Teach Assigned</h2>
        <?php
            require('connectdb.php');
            //B2:
            $sql = 'SELECT teachAssignedId, teacherLastName, teacherFirstName, subjectName, className FROM teacher, subject, teachassigned, class WHERE (teacher.teacherid = teachassigned.teacherid) AND (subject.subjectid = teachassigned.subjectid) AND (class.classid = teachassigned.classid)';
            mysqli_set_charset($dbcon, 'utf8');
            $result = mysqli_query($dbcon,$sql);
            //B3:
            echo '<font size="5">';
            echo '<table style="border: 2px solid"; align="center";>';
            echo '<tr style="border: 2px solid;"><th style="border: 2px solid">ID</th><th style="border: 2px solid">Teacher Name</th><th style="border: 2px solid">Class Name</th><th style="border: 2px solid">Subject Name</th><th style="border: 2px solid">Edit</th><th style="border: 2px solid">Delete</th></tr>';
            if(mysqli_num_rows($result)>0){
            while($row = mysqli_fetch_assoc($result)){
                echo '<tr style="border: 2px solid">';
                echo '<td style="border: 2px solid">'.$row['teachAssignedId'].'</td>';
                ...
                echo '<td style="border: 2px solid">'.'<a href="editTeachAssigned.php?id='.$row['teachAssignedId'].'">Edit</a>'.'</td>';
                echo '<td style="border: 2px solid">'.'<a href="deleteTeachAssigned.php?id='.$row['teachAssignedId'].'">Delete</a>'.'</td>';
                }
            }
            echo '</table>';
            echo '</font>';
            ...
            mysqli_close($dbcon);                
        ?>
        <button style="margin: 30px;" type="submit" name="submit" class="btn btn-secondary" onclick="location.href = 'addTeachAssigned.php'" >Add</button>
    </div>
