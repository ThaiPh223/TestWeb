...
<body>
    ...
    <div class="text-center main-content">
        <h2 style="margin-bottom:30px;">List Teacher</h2>
        <?php
            require('connectdb.php');
            //B2:
            $sql = 'SELECT teacherid, teacherLastName, teacherFirstName, teacherMail, teacherPhone FROM teacher';
            mysqli_set_charset($dbcon, 'utf8');
            $result = mysqli_query($dbcon,$sql);
            //B3:
            echo '<font size="5">';
            echo '<table style="border: 2px solid"; align="center";>';
            echo '<tr style="border: 2px solid;"><th style="border: 2px solid">ID</th><th style="border: 2px solid">Name</th><th style="border: 2px solid">Email</th><th style="border: 2px solid">Phone</th><th style="border: 2px solid">Edit</th><th style="border: 2px solid">Delete</th></tr>';
            if(mysqli_num_rows($result)>0){
            while($row = mysqli_fetch_assoc($result)){
                echo '<tr style="border: 2px solid">';
                echo '<td style="border: 2px solid">'.$row['teacherid'].'</td>';
                echo '<td style="border: 2px solid">'.$row['teacherFirstName'].' '.$row['teacherLastName'].'</td>';
                echo '<td style="border: 2px solid">'.$row['teacherMail'].'</td>';
                echo '<td style="border: 2px solid">'.$row['teacherPhone'].'</td>';
                echo '<td style="border: 2px solid">'.'<a href="editTeacher.php?id='.$row['teacherid'].'">Edit</a>'.'</td>';
                echo '<td style="border: 2px solid">'.'<a href="deleteTeacher.php?id='.$row['teacherid'].'">Delete</a>'.'</td>';
                }
            }
            echo '</table>';
            echo '</font>';
            ...
            mysqli_close($dbcon);                
        ?>
        <button style="margin: 30px;" type="submit" name="submit" class="btn btn-secondary" onclick="location.href = 'addTeacher.php'" >Add</button>
    </div>
