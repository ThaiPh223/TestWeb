...
<body>
    ...
    <div class="text-center main-content">
        <h2 style="margin-bottom:30px;">List Subject</h2>
        <?php
            require('connectdb.php');
            //B2:
            $sql = 'SELECT subjectid, subjectName, subjectDes, majorName FROM subject, major WHERE (subject.majorid = major.majorid)';
            mysqli_set_charset($dbcon, 'utf8');
            $result = mysqli_query($dbcon,$sql);
            //B3:
            echo '<font size="5">';
            echo '<table style="border: 2px solid"; align="center";>';
            echo '<tr style="border: 2px solid;"><th style="border: 2px solid">ID</th><th style="border: 2px solid">Name</th><th style="border: 2px solid">Description</th><th style="border: 2px solid">Major</th><th style="border: 2px solid">Edit</th><th style="border: 2px solid">Delete</th></tr>';
            if(mysqli_num_rows($result)>0){
            while($row = mysqli_fetch_assoc($result)){
                echo '<tr style="border: 2px solid">';
                echo '<td style="border: 2px solid">'.$row['subjectid'].'</td>';
                echo '<td style="border: 2px solid">'.$row['subjectName'].'</td>';
                echo '<td style="border: 2px solid">'.$row['subjectDes'].'</td>';
                echo '<td style="border: 2px solid">'.$row['majorName'].'</td>';
                echo '<td style="border: 2px solid">'.'<a href="editSubject.php?id='.$row['subjectid'].'">Edit</a>'.'</td>';
                echo '<td style="border: 2px solid">'.'<a href="deleteSubject.php?id='.$row['subjectid'].'">Delete</a>'.'</td>';
                }
            }
            echo '</table>';
            echo '</font>';
            ...
            mysqli_close($dbcon);                
        ?>
        <button style="margin: 30px;" type="submit" name="submit" class="btn btn-secondary" onclick="location.href = 'addSubject.php'" >Add</button>
    </div>
